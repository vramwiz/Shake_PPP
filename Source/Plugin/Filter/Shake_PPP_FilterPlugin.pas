...
procedure DebugLogRuntimeInput(Video: PFILTER_PROC_VIDEO;
  ...
  ObjectPositionUsed: Boolean; const Position: TAviUtl2ObjectPosition);
{$IFDEF DEBUG}
var
  CurrentTick: UInt64;
...
{$IFDEF DEBUG}
  ...
  begin
    DebugLog(Format(
      'Runtime input received: object=nil getOutputFunction=%s getOutputSucceeded=%s relativeParamAvailable=%s timeAxisRaw=%d positionSource=%s positionUsed=(%.6f,%.6f) bulge=(amount=%.3f shape=%.3f center=%.3f,%.3f gravity=%.3f direction=%.1f mass=%.3f tension=%.3f opacity=%.3f shading=%.3f light=%.1f highlight=%.3f).',
      [BoolToStr(GetOutputFunctionAvailable, True),
       ...
       PositionSource,
       Settings.PositionX, Settings.PositionY, BulgeSettings.Amount,
       BulgeSettings.Shape, BulgeSettings.CenterX, BulgeSettings.CenterY,
       BulgeSettings.Gravity, BulgeSettings.GravityDirection,
       BulgeSettings.Mass, BulgeSettings.Tension,
       BulgeSettings.OpacityResponse, BulgeSettings.ShadingStrength,
       BulgeSettings.LightDirection, BulgeSettings.HighlightStrength]));
    Exit;
  end;

  DebugLog(Format(
    'Runtime input received: objectId=%d effectId=%d layer=%d effectLayer=%d objectFlag=%d frame=%d frameRange=%d..%d time=%.6f getOutputFunction=%s getOutputSucceeded=%s relativeParamAvailable=%s timeAxisRaw=%d timeAxisUsed=%s positionSource=%s outputPosition=(%.6f,%.6f) relativePosition=(%.6f,%.6f) positionUsed=(%.6f,%.6f) bulge=(amount=%.3f shape=%.3f center=%.3f,%.3f gravity=%.3f direction=%.1f mass=%.3f tension=%.3f opacity=%.3f shading=%.3f light=%.1f highlight=%.3f).',
...
begin
  try
    ...
    BulgeSettings := CurrentBulgeRuntimeSettings;
    ...
    DebugLogRuntimeInput(Video, RuntimeSettings, BulgeSettings,
      ObjectPositionUsed, Position);
...
begin
  try
    ...
    try
      SettingsForm.SetBulgePreviewSettings(CurrentBulgeRuntimeSettings);
...
begin
  ...
  begin
    ...
    AddShakeFilterItems;
    AddBulgeFilterItems;
    AddGroup(InternalDataGroup, '内部データ', 0);
